...
def find_authors_nationalities():
    authors_no_nationalities = Author.objects.exclude(nationality__isnull=True)

    result = [
        f"{a.first_name} {a.last_name} is {a.nationality}"
        for a in authors_no_nationalities
    ]
...
def filter_authors_by_nationalities(nationality):
    ...
    for a in authors:
        if a.biography is None:
            result.append(f"{a.first_name} {a.last_name}")
        else:
            result.append(f"{a.biography}")
