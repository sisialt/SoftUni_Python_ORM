...
def product_quantity_ordered():
    ...
    ordered_products = (Product.objects.filter(orderproduct__quantity__gte=1)
                        .annotate(total_ordered_quantity=Sum('orderproduct__quantity'))
                        .order_by('-total_ordered_quantity')
                        )

    result = []
    for pr in ordered_products:
        result.append(f"Quantity ordered of {pr.name}: {pr.total_ordered_quantity}")
...
def ordered_products_per_customer():
    ...
    for order in orders:
        ...
        for pr in order.orderproduct_set.all():
            result.append(f"- Product: {pr.product.name}, Category: {pr.product.category.name}")
...
def filter_products():
    ...
    # products = (Product.objects.available_products().filter(price__gt=3)
    #           .order_by('-price', 'name'))
...
def give_discount():
    ...
    result = []
    for pr in Product.objects.filter(is_available=True).order_by('-price', 'name'):
        result.append(f"{pr.name}: {pr.price}lv.")
