...
class Task(models.Model):
    ...
    def recent_completed_tasks(cls, days: int):
        return cls.objects.filter(
            Q(is_completed=True) & Q(completion_date__gte=F('creation_date') - timedelta(days=days))
        )
...
class Exercise(models.Model):
    ...
    def get_exercises_with_difficulty_and_repetitions(cls, min_difficulty: int, min_repetitions: int):
        return cls.objects.filter(
            Q(repetitions__gte=min_repetitions) & Q(difficulty_level__gte=min_difficulty)
        )
