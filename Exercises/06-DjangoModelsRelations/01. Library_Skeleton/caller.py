...
def calculate_average_rating_for_product_by_name(product_name: str):
    ...
    rating_values = product.reviews.values_list('rating', flat=True)
    avg_rating = sum(rating_values) / rating_values.count()
...
def calculate_licenses_expiration_dates():
    ...
    for license in DrivingLicense.objects.order_by('-license_number'):
...
def register_car_by_owner(owner):
    ...
    first_not_related_registration = Registration.objects.filter(car__isnull=True).first()
    first_car_without_owner = Car.objects.filter(registration__isnull=True, owner=owner).first()

    # first_car_without_owner.owner = owner
    first_car_without_owner.registration = first_not_related_registration
    first_not_related_registration.registration_date = date.today()
    first_not_related_registration.car = first_car_without_owner

    first_car_without_owner.save()
    first_not_related_registration.save()

    return (f"Successfully registered {first_car_without_owner.model} to {first_car_without_owner.owner.name} "
            f"with registration number {first_not_related_registration.registration_number}.")
...
# print(register_car_by_owner(Owner.objects.get(name='Ivelin Milchev')))
