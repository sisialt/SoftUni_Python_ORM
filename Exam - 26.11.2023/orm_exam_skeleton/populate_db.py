    ...
    for _ in range(num_records):
        ...
        for field in model_fields:
            if hasattr(field, 'choices') and field.choices:
                ...
                field_values[field.name] = random.randint(0, 100)
            elif isinstance(field, FloatField):
                field_values[field.name] = random.randint(0, 10)
            elif isinstance(field, BooleanField):
                field_values[field.name] = random.choice([True, False])
            elif isinstance(field, EmailField):
                field_values[field.name] = f"{random.choice(['user', 'admin', 'customer','a','b','c', 'd'])}@example.com"
